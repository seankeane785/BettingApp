...
## D-014 — Model-owned auditable research coverage

Coverage diagnostics are generated by the deterministic model rather than inferred by React. Each family reports candidate/support records, exact matching benchmarks, produced candidates, and its unavailable reason. Manual prompts must start with specialist match-centre/statistics sources, attempt every enabled family, omit incomplete candidates, and request JSON Unicode slash-escaped URLs to preserve copy safety. Existing source validation, score thresholds, early-season treatment, and builder rules are intentionally unchanged.

## 2026-09-03 — Deterministic ResearchPack v1.4 source hierarchy

**Decision:** Route current-season aggregate research through SoccerStats for both supported competitions, then use FotMob, SofaScore, and official Premier League/EFL completed-match centres in that order only for missing explicit specialist match statistics. Prohibit WinDrawWin and reject averages as threshold evidence.

**Rationale:** A concrete hierarchy gives manual ChatGPT Search research practical routes beyond generic results pages while preserving the local-first boundary and every existing evidence, scoring, builder, validation, and availability gate.

**Consequence:** Missing exact observations still produce unavailable coverage; source routing cannot manufacture candidates or change probability calculations.

## Decision: source ownership is per evidence component

FootyStats owns an exact aggregate component when available; SoccerStats is consulted only if that exact component is absent. StatBunker is restricted to current-season team-level discipline, direct match pages follow FotMob → SofaScore → official match centre, and WhoScored is a Premier League-only team-shots fallback. Values for the same team, metric, scope, and sample are never merged; conflicting exact duplicates make only that component unreliable. This changes research instructions and diagnostics, not the schema or model.
