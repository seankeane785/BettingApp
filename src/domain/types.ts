...
export type ResearchRoute = 'official_fixture' | 'footystats' | 'soccerstats' | 'fotmob' | 'sofascore' | 'official_match_centre' | 'statbunker' | 'whoscored'
export type ResearchBlocker = 'missing_candidate_observation' | 'missing_support_observation' | 'missing_venue_observation' | 'missing_benchmark' | 'source_conflict' | 'no_exact_threshold_observation'
export interface MarketResearchAudit { marketGroup: MarketGroup; attempted: boolean; routesAttempted: ResearchRoute[]; candidateEvidenceFound: boolean; supportEvidenceFound: boolean; benchmarkFound: boolean; status: 'available' | 'unavailable'; firstBlockingReason: ResearchBlocker | null; sourceIds: string[] }
export interface DerivedTeamGoalsInput { matchesPlayed: number; goalsScored: number; goalsConceded: number; sourceIds: string[]; venue?: { matchesPlayed: number; goalsScored: number; goalsConceded: number; sourceIds: string[] } }
export interface Derived1x2FromGoals { strategy: 'derived_1x2_from_goals'; home: DerivedTeamGoalsInput; away: DerivedTeamGoalsInput; competitionCompletedFixtures: number; competitionPerTeamGoals: number; competitionSourceIds: string[]; sourceConflict: boolean }
export interface ResearchFixture { derived1x2FromGoals?: Derived1x2FromGoals }
export interface ResearchPack { packName: 'ResearchPack v1'; schemaVersion: '1.0.0' | '1.1.0' | '1.2.0' | '1.3.0' | '1.4.0' | '1.5.0'; fixturePackRef: { schemaVersion: '1.0.0'; fixtureDate: string }; generatedAt: string; dataStatus: 'synthetic' | 'real'; sources: SourceReference[]; competitionBenchmarks?: CompetitionBenchmark[]; marketResearchAudit?: MarketResearchAudit[]; fixtures: ResearchFixture[] }
export interface AnalysisPack { packName: 'AnalysisPack v1'; schemaVersion: '1.0.0'; generatedAt: string; fixturePack: FixturePack; researchPack: ResearchPack & { schemaVersion: '1.4.0' | '1.5.0' } }
...
export interface MarketCoverage { marketGroup: MarketGroup; status: 'analysed' | 'unavailable'; candidateMarketRecordsSupplied: number; supportingOnlyRecordsSupplied: number; matchingBenchmarksSupplied: number; candidateCount: number; missingEvidence: string[]; unavailableReason: string | null; researchAuditStatus?: 'available' | 'unavailable'; routesAttempted?: ResearchRoute[]; firstBlockingReason?: ResearchBlocker | null; resultEvidencePath?: 'direct_researched' | 'model_derived' | 'none' }
