...
Use team-level evidence only. Dedicated corners, cards, shots, and shots-on-target market evidence must never be inferred from goals. Exclude player markets, automated collection, prior-season data, predictions, selections, prices, bookmaker content, tipster content, implied probability, expected value, payouts, links, and stake advice. Prefer primary sources, cross-check contradictions, and mark fixture dataQuality insufficient when core evidence is absent. Set actual UTC generatedAt/retrievedAt values and output JSON only without Markdown.`
}
