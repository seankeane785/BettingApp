...
describe('AnalysisPack workflow', () => {
  ...
  it('requests the exact source hierarchy, contract, and every evidence family', () => { const prompt = buildAnalysisPackPrompt('2026-09-03', ['Premier League']); for (const phrase of ['FIXTURE DISCOVERY', 'FootyStats current-season league/team/statistics pages — preferred aggregate source', 'official Premier League/EFL completed-match centre', 'FotMob → SofaScore → official', 'WinDrawWin', 'average alone is never sufficient', 'manually counting only verified completed-match tables', '"sourceId": "non-empty kebab-case string"', '"url": "absolute HTTPS URL with a hostname"', 'never emit Markdown links or apply a domain allowlist', 'actual ISO 8601 UTC timestamp ending in Z', 'Match result', 'Double chance', 'Draw no bet', 'BTTS', 'Total goals', 'Team goals', 'Team to score', 'Clean sheet', 'Total corners', 'Team corners', 'Total cards', 'Team cards', 'Team shots', 'Team shots on target', 'supporting_only', 'candidate_market', 'required venue evidence', 'Manual availability controls', 'https:\\u002F\\u002Fwww.soccerstats.com\\u002Ftable.asp?league=england']) expect(prompt).toContain(phrase) })
})
