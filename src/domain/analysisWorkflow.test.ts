...
describe('AnalysisPack workflow', () => {
  ...
  it('requests the exact source hierarchy, contract, and every evidence family', () => { const prompt = buildAnalysisPackPrompt('2026-09-03', ['Premier League']); for (const phrase of ['FIXTURE DISCOVERY', 'SoccerStats is the mandatory primary', 'Premier League and Championship', 'FotMob direct completed-match page; SofaScore direct completed-match page; official Premier League or EFL completed-match centre', 'WinDrawWin is prohibited', 'average is not a threshold hit rate', 'exact manual count of verified observations', '"sourceId": "non-empty kebab-case string"', '"url": "absolute HTTPS URL with a hostname"', 'do not use a domain allowlist', '"retrievedAt": "ISO 8601 UTC timestamp ending in Z"', 'src-ch-results', 'Match result', 'double chance', 'draw-no-bet', 'BTTS', 'Total goals', 'Team goals', 'team to score', 'Clean sheets', 'Total and team corners', 'Total and team cards', 'Team shots:', 'Team shots on target:', 'supporting_only', 'candidate_market', 'required venue evidence', 'Manual market-availability dropdowns', '"url":"https:\\u002F\\u002Fwww.soccerstats.com\\u002Ftable.asp?league=england"', 'Never emit a Markdown-link source value']) expect(prompt).toContain(phrase) })
})
