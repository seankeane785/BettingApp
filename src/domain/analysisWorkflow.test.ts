...
describe('AnalysisPack workflow', () => {
  ...
  it('requests the exact source contract and every evidence family', () => { const prompt = buildAnalysisPackPrompt('2026-09-03', ['Premier League']); for (const phrase of ['FIXTURE DISCOVERY', '"sourceId": "non-empty kebab-case string"', '"url": "absolute HTTPS URL with a hostname"', 'do not use a domain allowlist', '"retrievedAt": "ISO 8601 UTC timestamp ending in Z"', 'src-ch-results', 'Match result', 'double chance', 'draw-no-bet', 'BTTS', 'Total goals', 'Team goals', 'team to score', 'Clean sheets', 'Total and team corners', 'Total and team cards', 'Team shots:', 'Team shots on target:', 'supporting_only', 'candidate_market', 'official competition match centres', 'every completed league match', 'required venue evidence', 'Manual market-availability dropdowns', '"url":"https:\\u002F\\u002Fwww.example.com\\u002Fpath"', 'Never emit a Markdown-link source value']) expect(prompt).toContain(phrase) })
})
