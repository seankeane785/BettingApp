...
describe('research workflow', () => {
  ...
  it('builds deterministic current-only v1.4 prompts with the shared policy', () => { const one = buildResearchPrompt(real, settings); expect(buildResearchPrompt(real, settings)).toBe(one); for (const text of ['"fixtureId": "championship-001"', 'schemaVersion "1.4.0"', 'competitionBenchmarks', 'candidate_penalty', 'descriptive_only', 'currentSeasonLeagueMatches', 'FootyStats', 'Premier League and Championship', 'FotMob → SofaScore → official', 'WinDrawWin', 'average alone is never sufficient', 'declared sourceIds', 'Do not include historicalMarketHitRates', '[team_shots_on_target]', 'player aggregation', 'https:\\u002F\\u002Fwww.soccerstats.com\\u002Ftable.asp?league=england', 'never emit Markdown links']) expect(one).toContain(text); expect(one).not.toContain(settings.referenceTimestamp) })
