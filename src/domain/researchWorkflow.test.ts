...
describe('research workflow', () => {
  ...
  it('builds deterministic current-only v1.3 prompts', () => { const one = buildResearchPrompt(real, settings); expect(buildResearchPrompt(real, settings)).toBe(one); for (const text of ['"fixtureId": "championship-001"', 'schemaVersion "1.3.0"', 'competitionBenchmarks', 'candidate_penalty', 'descriptive_only', 'currentSeasonLeagueMatches', 'shotsOnTarget', 'current-season league fixtures', 'Every populated item requires declared sourceIds', 'Do not include historicalMarketHitRates', 'Dedicated corners, cards, shots', 'player markets']) expect(one).toContain(text); expect(one).not.toContain(settings.referenceTimestamp) })
