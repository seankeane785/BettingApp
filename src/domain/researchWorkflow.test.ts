...
describe('research workflow', () => {
  ...
  it('builds deterministic current-only v1.4 prompts with the source hierarchy', () => { const one = buildResearchPrompt(real, settings); expect(buildResearchPrompt(real, settings)).toBe(one); for (const text of ['"fixtureId": "championship-001"', 'schemaVersion "1.4.0"', 'competitionBenchmarks', 'candidate_penalty', 'descriptive_only', 'currentSeasonLeagueMatches', 'shotsOnTarget', 'SoccerStats is the mandatory primary', 'Premier League and Championship', 'FotMob direct completed-match page; SofaScore direct completed-match page; official Premier League or EFL completed-match centre', 'WinDrawWin is prohibited', 'average as a threshold hit rate', 'exact manual count of verified observations', 'Every populated item requires declared sourceIds', 'Do not include historicalMarketHitRates', 'match result; double chance; draw no bet; both teams to score; total goals; team goals; team to score; clean sheet; total corners; team corners; total cards; team cards; team shots; team shots on target', 'mandatory relevant supporting_only', 'total/team corners', 'total/team cards', 'team shots on target', 'player markets', '"url":"https:\\u002F\\u002Fwww.soccerstats.com\\u002Ftable.asp?league=england"', 'Never emit a Markdown-link source value']) expect(one).toContain(text); expect(one).not.toContain(settings.referenceTimestamp) })
