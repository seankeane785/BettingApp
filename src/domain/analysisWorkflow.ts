...
RESEARCH RULES: For every discovered fixture, first identify credible current-season league match-centre/statistics pages for every completed league match involving each team. Start with specialist sources: official competition match centres and established football statistics providers. Only then use league results pages, and only for W/D/L and goal-based evidence where appropriate. Research every family below individually; never invent evidence or claim coverage merely to increase candidates. This is manual research only, with no automated collection. Build exact current-season hit-rate records only after verifying the underlying completed league matches and their dedicated match statistics. Set dataStatus to real. Use actual UTC generatedAt and retrievedAt timestamps and sources no older than 24 hours. Every source object must have exactly this contract and no other properties: { "sourceId": "non-empty kebab-case string", "url": "absolute HTTPS URL with a hostname", "title": "non-empty string", "retrievedAt": "ISO 8601 UTC timestamp ending in Z" }. In emitted JSON encode every slash in every source URL with JSON Unicode escapes, for example "url":"https:\\u002F\\u002Fwww.example.com\\u002Fpath". Never emit a Markdown-link source value such as [https://example.com](https://example.com). The JSON parser will decode \\u002F to /. Direct source-page paths, query strings, fragments, hyphens, and standard public domains are valid; do not use a domain allowlist. sourceId is the only ID field; never use id or include both id and sourceId. Every sourceId must match ^[a-z0-9]+(?:-[a-z0-9]+)*$; examples: "src-ch-results" and "official-match-centre". Cite every populated item using only declared sourceId values. Use current-season league data only: never prior seasons, cups, or friendlies. Use team-level evidence only. Never include odds, prices, bookmaker links, implied probability, value claims, payouts, stakes, tips, predictions, selections, or player markets. Mark dataQuality insufficient where required evidence cannot be credibly sourced. Do not insert neutral defaults.
