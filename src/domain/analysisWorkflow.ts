...
RESEARCH RULES: Research all supported team-level market families, but never invent evidence. Search for dedicated current-season specialist statistics from credible sources such as official competition match centres or established statistics providers before concluding that a specialist market is unavailable; this is manual research only, with no automated collection. Set dataStatus to real. Use actual UTC generatedAt and retrievedAt timestamps and sources no older than 24 hours. Every source object must have exactly this contract and no other properties: { "sourceId": "non-empty kebab-case string", "url": "absolute HTTPS URL with a hostname", "title": "non-empty string", "retrievedAt": "ISO 8601 UTC timestamp ending in Z" }. Direct source-page paths, query strings, fragments, hyphens, and standard public domains are valid; do not use a domain allowlist. sourceId is the only ID field; never use id or include both id and sourceId. Every sourceId must match ^[a-z0-9]+(?:-[a-z0-9]+)*$; examples: "src-ch-results" and "official-match-centre". Cite every populated item using only declared sourceId values. Use current-season league data only: never prior seasons, cups, or friendlies. Use team-level evidence only. Never include odds, prices, bookmaker links, implied probability, value claims, payouts, stakes, tips, predictions, selections, or player markets. Mark dataQuality insufficient where required evidence cannot be credibly sourced. Do not insert neutral defaults.
